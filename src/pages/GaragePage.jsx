...
function VehicleCard({ vehicle, onChanged }) {
  ...
  return (
    <div className="p-5 rounded-2xl" style={{ background: 'var(--color-surface)', border: `1.5px solid ${vehicle.is_default ? 'var(--color-brand)' : 'var(--color-border)'}`, boxShadow: 'var(--shadow-sm)' }}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3.5">
          <div className="p-3 rounded-xl" style={{ background: 'var(--color-brand-light)', color: 'var(--color-brand)' }}>
            <Icon size={22} />
          </div>
          <div>
        ...
        {vehicle.is_default ? (
          <span className="flex items-center gap-1 text-[12.5px] font-bold px-2.5 py-1 rounded-full" style={{ background: 'var(--color-brand-light)', color: 'var(--color-brand)' }}>
            <Star size={11} fill="currentColor" /> Default
          </span>
        ) : (
          <button
            onClick={async () => { await updateVehicle(vehicle.id, { is_default: true }); onChanged(); }}
            className="tap text-[12.5px] font-semibold px-2.5 py-1 rounded-full"
      ...
      <div className="flex justify-end mt-2">
        <button
          ...
          className="tap flex items-center gap-1 text-[12.5px] font-semibold px-2 py-1 rounded-lg"
    ...
    <div className="max-w-3xl mx-auto p-4 lg:p-8 pb-24 screen-fade">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="tap flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold text-white"
      ...
      {error && <div className="mt-5 p-4 rounded-xl text-sm" style={{ background: 'var(--color-rose-light)', color: 'var(--color-rose)' }}>{error}</div>}

      {vehicles && vehicles.length === 0 && (
        <div className="mt-8 p-10 text-center rounded-2xl" style={{ background: 'var(--color-surface)', border: '1px dashed var(--color-border-dark)' }}>
          ...
          <button onClick={() => navigate('/add-vehicle')} className="tap mt-5 px-6 py-3 rounded-xl text-sm font-bold text-white" style={{ background: 'var(--color-brand)' }}>
            Add your first EV
          </button>
        </div>
      )}
