...
function chargerIcon(charger, selected = false) {
  ...
  const html = `
    <div style="
      ...
      box-shadow:0 3px 10px rgba(15,23,42,${selected ? 0.4 : 0.25});
    ">
  ...
  const html = `
    <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
      <div style="width:16px;height:16px;border-radius:50%;background:${color};
        border:3px solid #fff;box-shadow:0 2px 6px rgba(15,23,42,0.3);"></div>
      ${label ? `<span style="font:600 10px Inter,sans-serif;color:#0f172a;background:rgba(255,255,255,0.9);padding:1px 6px;border-radius:6px;">${label}</span>` : ''}
    </div>`;
...
export default function MapView({
  ...
  return (
    ...
    >
      ...
      {userLocation && <Marker position={userLocation} icon={dotIcon('#2563eb', 'You')} />}
          ...
          {popup && (
            <Popup>
              <div style={{ fontFamily: 'Inter, sans-serif', minWidth: 160 }}>
                ...
                <div style={{ fontSize: 12, color: '#55606e', marginTop: 2 }}>
